...
- [x] 1. Andamiaje del proyecto (React + TypeScript + Vite)
...
- [x] 2. Tipos y datos sintéticos
  - [x] 2.1 Definir tipos en `src/types.ts` (`Persona`, `CasoUso`, `Asignacion`, `RiesgoNIST`, `SeedData`).
    - _Requisitos: modelo de datos, 1.2_
  - [x] 2.2 Crear `public/seed.json` con datos ficticios: ≥5 personas, ≥4 casos de uso cubriendo `bajo`/`medio`/`alto` (con `riesgoNIST` y descripción), ≥8 asignaciones.
    - _Requisitos: 1.1, 1.3, 1.5_

- [x] 3. Estado global y carga del seed
  - [x] 3.1 Implementar `src/state/store.tsx` con Context + `useReducer` (acciones LOAD_OK/LOAD_ERROR/ADD/UPDATE/DELETE).
    - _Requisitos: 3.2, 4.6, 5.4_
  - [x] 3.2 Cargar `seed.json` al iniciar con manejo de error (`status: loading|ready|error`).
    - _Requisitos: 1.1, 1.4_

- [x] 4. Lógica de cálculo pura (`src/lib/`)
  - [x] 4.1 `workload.ts`: `cargaPorPersona` y `estadoSemaforo` (<70 holgura, 70–100 ok, >100 sobrecarga).
    - _Requisitos: 4.1, 4.2, 4.3, 4.4, 4.7_
  - [x] 4.2 `risk.ts`: `capacidadPorRiesgo` (agrega por nivel, niveles ausentes = 0).
    - _Requisitos: 5.1, 5.3_
  - [x] 4.3 `validation.ts`: validación de asignación (obligatorios, dedicación 1–100, `hasta >= desde`).
    - _Requisitos: 3.4, 3.5, 3.6_

- [x] 5. Componentes reutilizables
  - [x] 5.1 `RiskBadge` (etiqueta/color por nivel NIST) y `TrafficLight` (semáforo).
    - _Requisitos: 2.2, 4.2, 4.3, 4.4_
  - [x] 5.2 `EmptyState` y `ErrorState`.
    - _Requisitos: 1.4, 2.3_

- [x] 6. Navegación y layout (`App`, `Nav`)
...
- [x] 7. Tabla de asignaciones (`AssignmentsTable`)
...
- [x] 8. Formulario crear/editar (`AssignmentForm`)
...
- [x] 9. Resumen de carga por persona (`WorkloadSummary`)
...
- [x] 10. Foco del equipo por riesgo (`TeamFocus`)
